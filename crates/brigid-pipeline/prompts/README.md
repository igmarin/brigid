...
| Chapter outline | `chapter_outline.md.j2` | `utils/tutorial_quality.py` `chapter_outline_instructions` | Injected into `write_chapter` to enforce fixed section order, diagram quotas, and grounding rules. Used with `--tutorial-style book`. | `lang`, `tier`, `diagram_level`, `need` | Markdown fragment with mandatory structure and rules. |
| Chapter outline (blog) | `chapter_outline_blog.md.j2` | `brigid` v1.1.0 | Blog-post variant: shorter, conversational structure with fewer mandatory sections. Used with `--tutorial-style blog` (default). | `lang`, `tier`, `diagram_level`, `need` | Markdown fragment with blog-style structure and rules. |
...
| Write architecture overview | `write_architecture_overview.md.j2` | `nodes.py` `WriteArchitectureOverview` | Chapter 0 overview for multi-app / engine-like monorepos. Used with `--tutorial-style book`. | `lang_note`, `project_name`, `summary`, `inventory`, `abstractions`, `relationships` | Markdown architecture overview. |
| Write architecture overview (blog) | `write_architecture_overview_blog.md.j2` | `brigid` v1.1.0 | Blog-post variant of the architecture overview: shorter, less formal. Used with `--tutorial-style blog` (default). | `lang_note`, `project_name`, `summary`, `inventory`, `abstractions`, `relationships` | Markdown architecture overview (blog style). |
