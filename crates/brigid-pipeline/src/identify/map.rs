...
pub async fn identify_map(
    ...
    let results = bounded_complete_with_budget(client, prompts, input.max_concurrency, progress)
        .await?;
...
/// model in [`brigid_core::estimate_budget`]). Files are packed in inventory
