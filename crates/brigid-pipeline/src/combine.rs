...
///
/// ```
/// use brigid_pipeline::combine::slugify_chapter_filename;
///
...
pub fn build_index_markdown(
    ...
    let path_diagram = brigid_core::learning_path_flowchart(&steps);
...
fn concept_map_flowchart(abstractions: &[Abstraction], relationships: &[Relationship]) -> String {
    use brigid_core::mermaid::{sanitize_label, stable_node_id};
...
mod tests {
    ...
    use brigid_core::{Chapter, RunConfig, Tier};
    use std::fs;
    ...
    fn temp_dir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("brigid-combine-{n}-{seq}"));
