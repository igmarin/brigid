...
//!
//! Assembles the Milestone 1 plan used by `brigid dry-run`. Parity with
...
use brigid_crawl::{CrawlError, CrawlOptions, crawl_local_with_options};
use thiserror::Error;
...
///
/// ```no_run
/// use brigid_pipeline::dry_run::dry_run;
///
...
/// [`brigid_crawl::crawl_local_with_options`]). Setup assessment and module
...
///
/// ```no_run
/// use brigid_crawl::CrawlOptions;
/// use brigid_pipeline::dry_run::dry_run_with_options;
///
...
mod tests {
    ...
    fn unique_temp_dir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("brigid-pipeline-dry-run-{nanos}-{seq}"));
    ...
    fn readme_secrets_are_redacted_before_assess_setup() {
        ...
        let redacted = brigid_core::redact_content(readme);
        assert_ne!(
            redacted.len(),
            readme.len(),
    ...
    fn dry_run_with_options_since_filters_files() {
        ...
        let opts = brigid_crawl::CrawlOptions::since("v1");
    ...
    fn dry_run_with_options_full_matches_dry_run() {
        ...
        let opts = brigid_crawl::CrawlOptions::full();
        let plan = dry_run_with_options(&dir, None, &BudgetConfig::default(), opts)
