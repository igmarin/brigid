//! Full pipeline orchestration for `brigid generate` (M4-CLI-1).
//!
...
use brigid_crawl::git_diff::{changed_files_since, deleted_files_since};
...
pub async fn run_generate(
    ...
    // --- Stage 5: Setup ---
    let mut setup: Option<brigid_core::SetupGuide> = None;
    let do_setup =
    ...
    // --- Stage 6: Overview ---
    let mut overview: Option<brigid_core::ArchitectureOverview> = None;
...
fn write_output_if_needed(
    ...
    setup: Option<&brigid_core::SetupGuide>,
    overview: Option<&brigid_core::ArchitectureOverview>,
...
///    (`.brigid-checkpoint-<slug>`), and calls [`run_generate`] with a scoped
...
fn write_each_app_index(
    ...
    content.push_str("Per-app tutorials generated by `brigid generate --each-app`.\n\n");
...
fn prerequisite_error(stage: &str, prerequisite: &str) -> GenerateError {
    GenerateError::Config(format!(
        "{stage} stage requires '{prerequisite}' to be complete -- run 'brigid {prerequisite}' or 'brigid generate' first"
    ))
}
...
/// [`brigid_core::SetupGuide`] and updates the checkpoint in place.
///
/// # Errors
///
...
pub fn run_combine_stage(
    ...
    let order = load_order(checkpoint)?;
    let chapters = load_chapters(store, checkpoint)?;
    let setup: Option<brigid_core::SetupGuide> = store
        .read_setup_guide(&store.dir, checkpoint)
        .ok()
        .flatten();
    let overview: Option<brigid_core::ArchitectureOverview> = store
        .read_architecture_overview(&store.dir, checkpoint)
        .ok()
        .flatten();
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("brigid-gen-{label}-{n}-{seq}"));
...
## Summary\nWe learned about {name}.\n"
    ...
    async fn resume_all_complete_skips_everything_writes_output() {
        ...
        let rels = RelationshipsResult::new(
            "summary".to_string(),
            vec![brigid_core::Relationship::new(
                0,
                1,
                "calls".to_string(),
                "calls".to_string(),
        ...
        let chapters = ChapterResult::new(vec![
            brigid_core::Chapter::new(0, 1, "Router", "# Router\n", Tier::S, "module", "f0"),
            brigid_core::Chapter::new(1, 2, "Store", "# Store\n", Tier::S, "module", "f1"),
            brigid_core::Chapter::new(2, 3, "Worker", "# Worker\n", Tier::S, "module", "f2"),
        ]);
