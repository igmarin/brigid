...
pub struct OverviewInput {
    ...
    /// ([`brigid_core::RelationshipsResult::project_summary`]).
    pub summary: String,
...
///
/// Module keys produced by [`brigid_core::module_key`] do not contain dots,
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("brigid-overview-ckpt-{n}-{seq}"));
        fs::create_dir_all(&dir).unwrap();
        dir
    }

    fn fresh_checkpoint() -> CheckpointV1 {
        let cfg = brigid_core::RunConfig::default();
        CheckpointV1::new(
            &cfg,
...
"
    ...
    async fn happy_path_multi_app_generates_overview() {
        ...
        let (result, invented) =
            write_architecture_overview(&client, &renderer, &input, &mut ProgressTracker::new(10))
                .await
...
";
    ...
    async fn overview_and_checkpoint_writes_file_and_marks_complete() {
        ...
        let result = overview_and_checkpoint(
            &client,
            &renderer,
            &store,
            &mut cp,
    ...
    async fn overview_and_checkpoint_resume_skips_when_complete() {
        ...
        let first = overview_and_checkpoint(
            &client,
            &renderer,
            &store,
            &mut cp,
        ...
        let second = overview_and_checkpoint(
            &client,
            &renderer,
            &store,
            &mut cp,
