...
///
/// ```no_run
/// use brigid_crawl::local::{crawl_local_with_options, CrawlOptions};
///
...
mod tests {
    ...
    fn missing_path_errors() {
        let err = crawl_local("/nonexistent/brigid-crawl-path-xyz").expect_err("missing");
    ...
    fn relative_posix_rejects_non_utf8_path() {
        ...
        let root = Path::new("/tmp/brigid-root");
        let bad = PathBuf::from(OsStr::from_bytes(b"/tmp/brigid-root/bad\xffname.txt"));
