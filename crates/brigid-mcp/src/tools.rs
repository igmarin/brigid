...
use brigid_core::{Abstraction, Chapter, Relationship, current_git_head};
...
/// The `#[tool]` macro generates `_tool_attr()` metadata functions that
/// already have doc comments, but the `tool_router()` function generated by
/// `#[tool_router]` does not, so we suppress `missing_docs` for this block.
#[allow(missing_docs)]
#[tool_router(vis = "pub")]
impl BrigidTools {
    ...
    #[tool(
        name = "find_abstraction_for_file",
        description = "Find the abstraction that owns a given source file. Returns the abstraction as JSON or null."
    )]
    ...
    #[tool(
        name = "abstraction_dependencies",
        description = "Get the dependency relationships for an abstraction by name. Returns outgoing and incoming edges with resolved names."
    )]
    ...
    #[tool(
        name = "files_for_abstraction",
        description = "List the source files belonging to an abstraction by name. Returns file paths from the crawl inventory."
    )]
    ...
    #[tool(
        name = "relevance_ranked_chapters",
        description = "Return the top-N chapters most relevant to a natural-language query. Uses keyword matching."
    )]
    ...
    #[tool(
        name = "chapter_for_file",
        description = "Find the chapter that explains a given source file. Composes file→abstraction→chapter lookup."
    )]
    ...
    #[tool(
        name = "list_abstractions",
        description = "List all abstractions, optionally filtered by kind. Returns abstraction references with index, name, tier, kind."
    )]
    ...
    #[tool(
        name = "is_checkpoint_stale",
        description = "Check whether the checkpoint is stale by comparing its git_commit to the current HEAD of the source directory."
    )]
...
mod tests {
    ...
    fn list_abstractions_returns_all() {
        let (dir, data) = full_data();
        let tools = BrigidTools::new(data);
        let result = tools.list_abstractions(Parameters(ListAbstractionsParams { filter: None }));
        let parsed: Vec<serde_json::Value> = serde_json::from_str(&result).unwrap();
