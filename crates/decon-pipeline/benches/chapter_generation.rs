...
use decon_pipeline::chapters::{DiagramLevel, write_single_chapter};
...
fn bench_write_single_chapter(c: &mut Criterion) {
    ...
    for n in [1usize, 4, 8] {
        ...
        let full_listing = decon_pipeline::chapters::select_chapter_file_context(
            &abs,
            &file_contents,
            80_000,
            12_000,
    ...
    for n in [1usize, 3, 5] {
        ...
        let order = decon_core::ChapterOrder::new((0..n).collect());
        ...
        group.bench_with_input(BenchmarkId::new("write_chapters", n), &n, |b, &_| {
            b.iter(|| {
                ...
                let config = decon_pipeline::chapters::ChaptersConfig::default();
                let result = rt.block_on(decon_pipeline::chapters::write_chapters(
                    black_box(&client),
                    black_box(&renderer),
                    black_box(&identify),
                    black_box(&order),
