...
fn format_abstraction_listing(abstractions: &[decon_core::Abstraction]) -> String {
    abstractions
        .iter()
        .enumerate()
        .map(|(idx, a)| {
...
mod tests {
    ...
    use decon_core::{Abstraction, Relationship, RunConfig, Tier};
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("decon-ord-ckpt-{n}-{seq}"));
    ...
    fn five_abstractions() -> Vec<Abstraction> {
        vec![
            ...
            Abstraction {
                ...
                file_indices: vec![3],
                tier: Tier::S,
                kind: decon_core::AbstractionKind::new("config"),
                apps: vec![],
