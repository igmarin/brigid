...
use decon_core::{Abstraction, IdentifyResult, ProgressTracker, RunConfig, module_key};
...
///   still exist on disk (from [`decon_crawl::git_diff::changed_files_since`]).
/// * `deleted_files` — relative paths of files deleted since the ref (from
///   [`decon_crawl::git_diff::deleted_files_since`]).
...
mod tests {
    use super::*;
    use decon_core::{Abstraction, AbstractionKind, RunConfig, Tier};
    ...
    fn one_per_batch_budget() -> decon_core::BudgetConfig {
        decon_core::BudgetConfig {
