...
//!
//! [`StageId::Identify`]: decon_core::StageId::Identify
...
pub async fn identify_with_cancellation(
    client: &dyn decon_llm::LlmClient,
...
mod tests {
    ...
    use crate::prompts::PromptRenderer;
    use decon_core::{BudgetConfig, RunConfig, Tier};
...
";
    ...
    async fn cancel_before_reduce_writes_partial_no_identify_complete() {
        ...
        impl LlmClient for CancelAfterMap {
            async fn complete(&self, prompt: &str) -> Result<String, decon_llm::LlmError> {
        ...
        let batches = vec![
            ...
            CandidateBatch {
                ...
                candidates: vec![CandidateAbstraction {
                    ...
                    file_indices: vec![1],
                    tier: Tier::M,
                    kind: decon_core::AbstractionKind::new("util"),
                    apps: vec![],
                    entry_files: vec![],
                    batch_idx: 1,
                }],
