...
/// at call sites that only depend on `decon-pipeline`.
...
pub enum RelationshipsError {
    ...
    Extract(#[from] decon_core::ExtractError),
...
pub async fn analyze_relationships(
    ...
    identify: &decon_core::IdentifyResult,
    ...
    let relationships = raw
        ...
        .into_iter()
        .map(|r| {
            decon_core::Relationship::new(r.from_abstraction, r.to_abstraction, r.label, r.kind)
        })
        .collect();
...
mod tests {
    ...
    use decon_core::{Abstraction, IdentifyResult, RunConfig, Tier};
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("decon-rel-ckpt-{n}-{seq}"));
...
";
    ...
    fn save_relationships_result_populates_and_marks_complete() {
        ...
        let result = RelationshipsResult::new(
            "A summary.",
            vec![decon_core::Relationship::new(0, 1, "calls", "calls")],
        );
