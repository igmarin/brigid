...
//! Live full-pipeline smoke test for `decon generate` on a real monorepo.
//!
...
//! ```
//!
//! The test invokes the `decon` CLI binary via [`assert_cmd`] to run the full
//! `generate` pipeline on the `umbrella` monorepo fixture, then runs `decon
//! eval` and validates the checkpoint. It is budget-capped via
//! `DECON_MAX_LLM_CALLS` (default `20`) and uses a disk cache under
//! `target/decon-llm-cache` so re-runs with unchanged prompts are free.
...
use decon_core::StageId;
use decon_pipeline::CheckpointStore;

/// Maximum LLM calls the full generate pipeline may make. Overridable via
/// `DECON_MAX_LLM_CALLS`; defaults to a conservative `20`.
...
/// Full-pipeline live smoke test: runs `decon generate` on the umbrella
/// monorepo, then `decon eval`, and validates the checkpoint.
#[test]
#[ignore]
fn live_full_generate_smoke() {
    ...
    let output_dir = PathBuf::from("/tmp/decon-smoke-output");
    let checkpoint_dir = PathBuf::from("/tmp/decon-smoke-checkpoint");
    ...
    let generate_output = Command::cargo_bin("decon")
        .expect("decon binary found")
        .arg("generate")
        .arg("--dir")
        .arg(&fixture)
        .arg("--output-dir")
        ...
        .arg("5")
        .env("DECON_MAX_LLM_CALLS", budget.to_string())
        .env("DECON_LLM_CACHE_DIR", &cache)
    ...
    let eval_output = Command::cargo_bin("decon")
        .expect("decon binary found")
        .arg("eval")
        .arg("--out")
        .arg(&output_dir)
