...
//!
//! Measures [`decon_llm::DiskCache::get`] for both cache hits and misses, and
//! [`decon_llm::DiskCache::put`] with varying payload sizes. The cache uses
//! async `tokio::fs` I/O (M6-PERF-3), so each iteration drives a tokio runtime
//! via `block_on`.

use criterion::{BenchmarkId, Criterion, black_box, criterion_group, criterion_main};
use decon_llm::{CacheKeyInput, DiskCache, cache_key};
...
fn temp_root() -> std::path::PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("decon-bench-cache-{n}"))
}
